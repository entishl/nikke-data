      ...
      <div class="control-group">
        ...
        <div class="filter-items-container">
          <div v-for="union in unions" :key="union.id" class="filter-item">
            <input
              type="checkbox"
              :id="'union-electronic-' + union.id"
              :value="union.id"
              v-model="selectedUnionIds"
            />
            <label :for="'union-electronic-' + union.id">{{ union.name }}</label>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="!loading && !error" class="table-container">
      <table>
        <thead>
          <tr v-if="!showDetails">
            <th>玩家</th>
            <th v-for="char in electronicCharacterHeaders" :key="char.id" @click="handleSort(String(char.id))" class="sortable">
              {{ char.name_cn }}
          ...
          <template v-else>
            <tr>
              ...
              <th v-for="char in electronicCharacterHeaders" :key="char.id" :colspan="detailAttributes.length" class="character-header">
                {{ char.name_cn }}
              </th>
            </tr>
            <tr>
              <template v-for="char in electronicCharacterHeaders" :key="char.id + '-details'">
        ...
        <tbody>
          <tr v-for="player in sortedPlayers" :key="player.id">
            ...
            <template v-if="!showDetails">
              <td v-for="headerChar in electronicCharacterHeaders" :key="headerChar.id">
                {{ getFormattedDegree(player.name, headerChar.id) }}
              </td>
            </template>
            <template v-else>
              <template v-for="headerChar in electronicCharacterHeaders" :key="headerChar.id + '-details-body'">
...
const fetchData = async () => {
  ...
  try {
    ...
    const [playersRes, uniqueCharsRes, charDataRes] = await Promise.all([
      ...
      axios.get('/api/characters/', { params: { ...params, element: 'Electronic' } })
    ]);
...
// Filter unique characters to get only electronic characters that are also marked as is_C
const electronicCharacterHeaders = computed(() => {
  const electronicCharsFromData = new Set(characterData.value.filter(c => c.is_C).map(c => c.character_id));
  return uniqueCharacters.value.filter(char =>
    char.element === 'Electronic' && electronicCharsFromData.has(char.id)
  ).sort((a, b) => a.id - b.id);
});
