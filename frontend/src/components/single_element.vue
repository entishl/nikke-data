      ...
      <div class="control-group">
        ...
        <div class="filter-items-container">
          <div v-for="union in unions" :key="union.id" class="filter-item">
            <input
              type="checkbox"
              :id="'union-filter-' + element + union.id"
              :value="union.id"
              v-model="selectedUnionIds"
            />
            <label :for="'union-filter-' + element + union.id">{{ union.name }}</label>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="!loading && !error" class="table-container">
      <table>
        <thead>
          <tr v-if="!showDetails">
            <th>玩家</th>
            <th v-for="char in elementCharacterHeaders" :key="char.id" @click="handleSort(String(char.id))" class="sortable">
              {{ char.name_cn }}
          ...
          <template v-else>
            <tr>
              ...
              <th v-for="char in elementCharacterHeaders" :key="char.id" :colspan="detailAttributes.length" class="character-header">
                {{ char.name_cn }}
              </th>
            </tr>
            <tr>
              <template v-for="char in elementCharacterHeaders" :key="char.id + '-details'">
        ...
        <tbody>
          <tr v-for="player in sortedPlayers" :key="player.id">
            ...
            <template v-if="!showDetails">
              <td v-for="headerChar in elementCharacterHeaders" :key="headerChar.id">
                {{ getFormattedDegree(player.name, headerChar.id) }}
              </td>
            </template>
            <template v-else>
              <template v-for="headerChar in elementCharacterHeaders" :key="headerChar.id + '-details-body'">
...
const fetchData = async () => {
  ...
  try {
    ...
    const [playersRes, uniqueCharsRes, charDataRes] = await Promise.all([
      ...
      axios.get('/api/characters/', { params: { ...params, element: props.element } })
    ]);
...
// Filter unique characters to get only element characters that are also marked as is_C
const elementCharacterHeaders = computed(() => {
  const elementCharsFromData = new Set(characterData.value.filter(c => c.is_C).map(c => c.character_id));
  return uniqueCharacters.value.filter(char =>
    char.element === props.element && elementCharsFromData.has(char.id)
  ).sort((a, b) => a.id - b.id);
});
...
.element-table {
  ...
  --theme-color-dark: v-bind(props.themeColorDark);
  padding: 20px;
  background: #f9f9f9;
  border-radius: 8px;
}
